...
C<single()> answers the question: does this Cisco::Reconfig object uniquely
specify a single line?  In the example above, the object for word
C<ip> (above) does not but the object for the word C<address> does.
...
Cisco::Reconfig objects are highly self-referential.  They will not deallocate
themselves when they go out of scope.  The C<destory()> method will
cause the entire nest of them to loose all their values which will
allow them to be deallocated.  Use C<destory()> only if you're reading
...
Copyright (C) 2002 David Muir Sharnoff <muir@idiom.com>
